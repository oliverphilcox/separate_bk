...
- `datafile`: This specifies the input datafile, which contains {k1, k2, k3, S(k1,k2,k3)}, where S is a dimensionless shape function (or set of shapes). The input k values should span the entire range of interest.
...
- `optimal_weights`: If `true`, the code computes the overall weight-vector analytically (and exactly) instead of learning them from the data directly.
