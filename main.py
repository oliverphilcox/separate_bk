...
def run_experiment(config, test_id):
    ...
    OPTIMAL_WEIGHTS = config['model_params']['optimal_weights']
    ...
    train_indices = all_indices[:len(all_indices)//10*9]
    valid_indices = all_indices[len(all_indices)//10*9:]
    ...
    for this_num_terms in range(1,num_terms+1):
        ...
        # Check the outputs
        cosine, ratio, weights = sep_bk_nn.get_cosine(X_all, y_all)
        print('Estimated cosine is {} '.format(cosine))
        print('Estimated ratio is {}'.format(ratio))
        if np.abs(1-np.min(cosine))<np.abs(1-threshold):
    ...
    sep_bk_nn.save_model(model_path, this_num_terms, SYMM_KIND, ADD_BIAS, OPTIMAL_WEIGHTS, weights)
...
def main():
    ...
    if args.test_id:
        ...
        for test_id, results in all_results.items():
            print(f"{test_id}: COSINE = {results['cosine']}, RATIO = {results['ratio']}")
